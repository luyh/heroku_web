...
from zkb_state import zkb,connect_chrome,quere_up_task
...
class Taobao_thread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            elif zkb.is_connectedChrome_loginZKB_receivedTask():
                msm('赚客班接到任务')
                # target_pic = image.url_to_image( zkb.taskInfo['main_link'] )
                # find = False
                # for page in range( 15 ):
                #
                #     url = taobao.get_url( hpg.taskInfo['keyword'], page * 44 )
